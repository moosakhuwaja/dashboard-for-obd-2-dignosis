    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                login_user(user)
                token = jwt.encode({'email': user.email, 'exp': datetime.utcnow() + timedelta(minutes=30)}, app.secret_key)
...
def signup():
    if request.method == 'POST':
        ...
        if existing_user:
            flash('The email address is already registered. Please choose another one.', 'error')
        else:
